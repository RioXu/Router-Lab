...
void update(bool insert, RoutingTableEntry entry)
{
	for (int i = 0; i < table.size(); i++)
	{
		if (entry.addr == table[i].addr && entry.len == table[i].len)
		{
			if (insert)
			{
				if (entry.nexthop == table[i].nexthop)
					table[i] = entry;
				else
				{
...
{
	int len = 0, index = -1;
	uint32_t max_ul = 0xffffffff;
	for (int i = 0; i < table.size(); i++)
	{
		uint32_t mask = max_ul >> (32 - table[i].len);
		if ((addr & mask) == (table[i].addr & mask) && (table[i].len > len))
		{
			len = table[i].len;
			index = i;
		}
